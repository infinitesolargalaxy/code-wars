...
describe('validatePIN', () => {
  ...
  it('should handle all edge cases', () => {
    ...
    for (const s of tests) assert.strictEqual(validatePIN(s), false, `Wrong output for '${s}'`);
  });

  it('should work with random input', () => {
    const randint = (a, b) => ~~(Math.random() * (b - a + 1)) + a;
    const randValidPin = () => {
      const length = Math.random() < 0.5 ? 4 : 6;
      const digits = '0123456789';
      return Array.from({ length }, () => digits[randint(0, digits.length - 1)]);
    };
