...
describe('Tests', () => {
  ...
  it('Long Test', () => {
    ...
    jadenQuotes.forEach((quote) => {
      assert.strictEqual(quote[0].toJadenCase(), quote[1]);
    });
  });
});
...
String.prototype.toJadenCase = function () {
  ...
  // Map string, uppercasing first letter
  // Glue back into string

  const upperCaseFirstLetter = (string) => `${string.charAt(0).toUpperCase()}${string.slice(1)}`;
  const words = this.split(' ');
  const jadenWords = words.map((string) => upperCaseFirstLetter(string));
