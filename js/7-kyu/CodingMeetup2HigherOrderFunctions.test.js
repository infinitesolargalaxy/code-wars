...
//     newCoder.greeting = `Hi ${coder.firstName}, what do you like the most about ${coder.language}?`;
//     return newCoder;
//   });
// }
...
describe('Tests', () => {
  it('test', () => {
    ...
    assert.deepEqual(greetDevelopers(list1), greetDevelopersSolution(list1));
    ...
    // Working solution
    function greetDevelopersSolution(list) {
      return list
        .map((dev) => ({
          ...dev,
          greeting:
    ...
    for (let i = 0; i < 20; i++) {
      ...
      console.log(`Random test #${i + 1}, List of developers: ${JSON.stringify(listFullCopy)}`);
      assert.deepEqual(greetDevelopers(listFullCopy), greetDevelopersSolution(listFullCopy));
    }
  });
});
